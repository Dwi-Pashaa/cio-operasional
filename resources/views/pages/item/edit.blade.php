    ...
    <div class="col-12 col-md-8 col-lg-6 mx-auto">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                <h3 class="card-title fw-bold text-dark mb-0">Edit Barang</h3>
                ...
                <form action="{{ route('item.update', $item->id) }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="unit" class="form-control @error('unit') is-invalid @enderror" value="{{ old('unit', $item->unit) }}" required>
                            @error('unit')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" step="any" name="default_price" class="form-control @error('default_price') is-invalid @enderror" value="{{ old('default_price', $item->default_price) }}">
                            @error('default_price')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Kode SKU / Barcode</label>
                    ...
                    <div class="mb-4">
                        <label class="form-check form-switch cursor-pointer">
                            ...
                            <span class="form-check-label fw-semibold">Barang Aktif</span>
                        </label>
                    </div>

                    <div class="d-flex justify-content-end gap-2">
                        <a href="{{ route('item.index') }}" class="btn btn-outline-secondary">Batal</a>
                        <button type="submit" class="btn btn-primary fw-semibold px-4">Update Barang</button>
                    </div>
                </form>
            </div>
        </div>
